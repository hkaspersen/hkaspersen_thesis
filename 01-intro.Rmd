...
In greedy assembly algorithms, the reads are iteratively joined together in decreasing order of quality of the overlaps [@Simpson2015]. Conflicting read overlaps are ignored, and the contigs grow by adding new reads or joining two or more contigs. The major limitation of this method is that it cannot handle repeated genomic regions [@Simpson2015]. Due to bigger and more complex genomes being sequenced, graph-based algorithms were implemented to handle these issues. Graph-based approaches is separated into overlap-layout-concensus (OLC), De Bruijn, and string graphs. OLC is the simplest of these three, where each read is a vertex in the graph and overlapping sequences are linked with an edge. First, overlapping sequences are identified. This is the most time-consuming part of the assembly. Then, the graph is created, and the correct orientation of the reads as well as their order is identified. Lastly, a concensus sequence is computed and output as contigs [@Simpson2015, @Myers2005]. This method is usually only used for small genomes, and is mostly replaced by De Bruijn graph methods for larger and more complex genomes. 
...
De Bruijn graph algorithms is based on separating the reads into _k_-mers of a specific length, then creating a graph by identifying overlaps, similar to the greedy algorithm. Here, the algorithm "walks" through the graph, and identifies the optimal path where each edge in the graph is visited only once, also known as an Eulerian path [@Pevzner2001]. 
