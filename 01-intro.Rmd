...
Certain thresholds have been defined when categorizing isolates from AST results, based on minimum inhibitory concentration (MIC) values. MIC values are defined as the concentration of antimicrobial where growth is inhibited.





The epidemiological cut-off values (ECOFF), as defined by the European Committee on Antimicrobial Susceptibility Testing (EUCAST, www.eucast.org), are used to define the isolate as resistant or sensitive. The ECOFF is defined from the distribution of MIC-values (Figure \@ref(img:ecoff)). 

```{r, fig.cap="\\label{fig:ecoff}Distribution of MIC-values showing wild-type, intermediate and resistant bacterial sub-populations. Reprinted with permission from Veterinary Microbiology \\citep{Bywater2006}.", echo=FALSE, message=FALSE, fig.pos="H", fig.align="center"}
knitr::include_graphics("images/ecoff.jpg")
```
...
Following the sequencing of the Human Genome in 2004 with Sanger sequencing, a multi-million dollar sequencing initiative formed [@Schloss2008], resulting in the emergence of multiple high-throughput sequencing (HTS) technologies. Throughout the years, Illumina has quickly become the dominant short-read technology by which high-throughtput sequencing have developed. With the MiSeq, HiSeq and NextSeq sequencers, HTS has rapidly become both cheaper and more efficient. The MiSeq sequencer has been readily used for bacterial genomes, due to the relatively fast instrument run time and the small size of the genomes [@Reuter2015].

The process of HTS builds upon that of Sanger sequencing. First, the genomic DNA (gDNA) is fragmented and enriched either by PCR or by hybridization [@Morey2013], through a process called library preparation. Here, the genomic DNA fragments are adapter ligated and prepared for sequencing. Then, the prepared fragments are placed on a flowcell and a clonal array is generated by bridge PCR [@Morey2013], see Figure \@ref(fig:seqmet). This is done to amplify the signal for the sequencer to detect. Afterwards, sequencing-by-synthesis is performed to identify the bases in each DNA fragment, summarised in Figure \@ref(fig:seqmet).

```{r, fig.cap="\\label{fig:seqmet}Steps in Illumina sequencing; A: Generation of the clonal array by bridge PCR. The DNA fragments are denatured, then bind to the flow cell oligonucleotides by the attached adapters. The free end of the DNA fragment then hybridize to an adjacent oligonucleotide. Then, the polymerase extends the oligonucleotide to form a double-stranded fragment. The fragments are then denatured, resulting in two separate fragments relatively close to each other. This is then repeated to form clusters of fragments on the flowcell. B: Sequencing by synthesis, which takes place in the clonal clusters created in A. Bases are added to the reaction, and only one base can be added at a time due to a 3' block. Fluorescence is emitted after incorporation, and is registered by the instrument. Lastly, the 3' block is removed and the process is repeated. Modified and reprinted with permission from Elsevier \\citep{Morey2013}.", echo=FALSE, message=FALSE, fig.pos="H", fig.align="center", out.width="90%"}
...
The main analysis pathway of culture-based genomic data is shown in Figure \@ref(fig:bioinfo). Briefly, DNA is extracted from pure cultures and sequenced. Then, the quality controlled reads are either assembled into contigs, or a read mapping method is used. Then, downstream analyses like network analyses, resistance profiling and ordination may be done. 
